...
function id_arr = init_arr(A)
    ...
    A = logical(A);
    id_arr = zeros(size(A),'uint32'); % 
    id_arr(A) = uint32(1:nnz(A)); % replace true entries with sequential values. nnz-> number of non zero values
end
...
function conn_map = get_conn_map(vox_arr)
    % z connectors (rows)
    [Rows,Cols,Slices] = size(vox_arr);
    mask = false(size(vox_arr));
    ...
    if Rows > 1
        A1 = vox_arr(1:end-1,:,:); B1 = vox_arr(2:end,:,:); % get two copies shifted by one
        M1 = (A1>0) & (B1>0);
    ...
    % x connectors (cols)
    if Cols > 1
        A2 = vox_arr(:,1:end-1,:); B2 = vox_arr(:,2:end,:);
        M2 = (A2>0) & (B2>0);
    ...
    % y connectors (layers)
    if Slices > 1
        A3 = vox_arr(:,:,1:end-1); B3 = vox_arr(:,:,2:end);
        M3 = (A3>0) & (B3>0);
...
function voxel_config = get_voxel_config(nodes, vox_id)
    ...
    voxel_config  = [
    %face1
                b1,cl1,offset,0,3*pi/4, l_45, e, b, h;
                b1,m1,offset,0,pi/2, l, e, b, h;
                b1,cr1,offset,0,pi/4,l_45, e, b, h;
                cl1,t1,offset,0,pi/4,l_45, e, b, h;
                m1,t1,offset,0,pi/2, l, e, b, h;
                cr1,t1,offset,0,3*pi/4, l_45, e, b, h;
                cl1,m1,offset,0,0, l, e, b, h;
                m1,cr1,offset,0,0, l, e, b, h;
    %face 2
                cf2,b2,offset,pi/2,-pi/4,l_45, e, b, h;
                cf2,m2,offset,pi/2,0, l, e, b, h;
                cf2,t2,offset,pi/2,pi/4,l_45, e, b, h;

                b2,cb2,offset,pi/2,pi/4,l_45, e, b, h;
                m2,cb2,offset,pi/2,0, l, e, b, h;
                t2,cb2,offset,pi/2,-pi/4,l_45, e, b, h;

                b2,m2,offset,pi/2,pi/2, l, e, b, h;
                m2,t2,offset,pi/2,pi/2, l_45, e, b, h;

   %face 3
                cl3,b3,offset,0,-pi/4,l_45, e, b, h;
                cl3,m3,offset,0,0,l, e, b, h;
                cl3,t3,offset,0,pi/4,l_45, e, b, h;

                b3,cr3,offset,0,pi/4,l_45, e, b, h;
                m3,cr3,offset,0,0,l, e, b, h;
                t3,cr3,offset,0,-pi/4,l_45, e, b, h;

                b3,m3,offset,0,pi/2,l, e, b, h;
                m3,t3,offset,0,pi/2,l, e, b, h;

   %face 4
                cf4,b4,offset,pi/2,-pi/4,l_45, e, b, h;
                cf4,m4,offset,pi/2,0,l, e, b, h;
                cf4,t4,offset,pi/2,pi/4,l_45, e, b, h;

                b4,cb4,offset,pi/2,pi/4,l_45, e, b, h;
                m4,cb4,offset,pi/2,0,l, e, b, h;
                t4,cb4,offset,pi/2,-pi/4,l_45, e, b, h;

                b4,m4,offset,pi/2,pi/2,l, e, b, h;
                m4,t4,offset,pi/2,pi/2,l, e, b, h;

   %face 5 (top)

                f5,l5,-pi/2+offset,0,3*pi/4,l_45, e, b, h;
                f5,m5,-pi/2+offset,0,pi/2,l, e, b, h;
                f5,r5,-pi/2+offset,0,pi/4,l_45, e, b, h;

                l5,bk5,-pi/2+offset,0,pi/4,l_45, e, b, h;
                m5,bk5,-pi/2+offset,0,pi/2,l, e, b, h;
                r5,bk5,-pi/2+offset,0,3*pi/4,l_45, e, b, h;

                l5,m5,-pi/2+offset,0,0,l, e, b, h;
                m5,r5,-pi/2+offset,0,0,l, e, b, h;

%face 6 (bottom)
                f6,l6,-pi/2+offset,0,3*pi/4,l_45, e, b, h;
                f6,m6,-pi/2+offset,0,pi/2,l, e, b, h;
                f6,r6,-pi/2+offset,0,pi/4,l_45, e, b, h;

                l6,bk6,-pi/2+offset,0,pi/4,l_45, e, b, h;
                m6,bk6,-pi/2+offset,0,pi/2,l, e, b, h;
                r6,bk6,-pi/2+offset,0,3*pi/4,l_45, e, b, h;

                l6,m6,-pi/2+offset,0,0,l, e, b, h;
                m6,r6,-pi/2+offset,0,0,l, e, b, h;
    ]; 

end

function id_pairs = find_conn_nodes_x(nodes, r,c,s, id_arr)
...
function id_pairs = find_conn_nodes_y(nodes, r,c,s, id_arr)
...
function id_pairs = find_conn_nodes_z(nodes, r,c,s, id_arr)
    %assumes you checked that there was a neighbor in x (cols)
    vox_id1 = id_arr(r,c,s);
...
function conn_config = get_connector_edges(conn_map, id_arr, nodes)
    ...
    conn_config = [];
    if any(conn_map.x)
        for id = 1:nnz(conn_map.x)
            [rx,cx,sx] = find(id_arr==id, 1);
            conn_config = [conn_config; find_conn_nodes_x(nodes,rx,cx,sx,id_arr)];
        end
    end

    if any(conn_map.y)
        for id = 1:nnz(conn_map.y)
            [ry,cy,sy] = find(id_arr==id, 1);
            conn_config = [conn_config; find_conn_nodes_y(nodes,ry,cy,sy,id_arr)];
        end
    end
    if any(conn_map.z)
        for id = 1:nnz(conn_map.z)
            [rz,cz,sz] = find(id_arr==id, 1);
            conn_config = [conn_config; find_conn_nodes_y(nodes,rz,cz,sz,id_arr)];
        end
    end
